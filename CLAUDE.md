...
- [OBSERVED 2026-04-17] **Codex benchmark framing**: Codex can treat benchmark questions as requests to inspect the current machine and stall or time out. `benchmark_core.runner._build_effective_prompt()` now wraps Codex prompts in explicit benchmark-mode instructions; in Bridge-Aided mode the wrapper preserves the `TOOL_CALL` protocol instead of telling Codex to answer directly.
