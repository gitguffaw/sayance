...
- [OBSERVED 2026-04-25] **Codex benchmark framing**: Codex can treat benchmark questions as requests to inspect the current machine and stall or time out. Do not solve that by adding shell/POSIX guidance to Unaided prompts; that contaminates naked baselines. Unaided Codex now receives the same raw question text as other providers. `benchmark_core.runner._codex_benchmark_prompt()` is reserved for Bridge-Aided mode, where it permits the bridge's `sayance-lookup <utility>` bash invocation.
