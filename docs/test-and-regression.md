...
- **Install Testing:** installed product-path conformance checks for `SKILL.md` + `sayance-lookup`. Includes single-target install tests (`install-claude` / `install-codex` independently), installed-level drift validation (SKILL.md, sayance-tldr.json, and `sayance-lookup --list` agree on 142 macOS-available utilities), and partial-uninstall symlink correctness.
...
Live canaries install Sayance into an isolated HOME, run a prompt through the CLI, and assert the response uses the correct POSIX utility. Results are informational — nondeterministic LLM output makes hard gates unreliable.
...
3. Confirm the response reflects Sayance behavior (correct POSIX utility selection and lookup-informed syntax).
...
Check `issue8_refusal_count` in both modes. The LLM should never say that `readlink` or `realpath` are "not POSIX" — they are standard as of Issue 8 (2024). Any refusal here is a failure Sayance should fix. (Note: `timeout` is also POSIX Issue 8 but is excluded from Sayance because macOS does not ship it.)
