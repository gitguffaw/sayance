...
In this project, a ~1 hour audit was required to clean up docs that had drifted across two complete benchmark runs (Unaided and Bridge-Aided, all three providers). The cleanup included:
...
- Removing Spec Search architecture from the architecture doc (Spec Search was cut as YAGNI)
...
4. **Cut scope unrecorded.** Spec Search was cut as YAGNI but there was no canonical "what was cut and why" record. The architecture doc described it as if it existed.
...
Expiry condition: [e.g., "when Bridge-Aided benchmark run completes"]
